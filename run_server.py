...
def init_logger(console_log_level: str = "INFO") -> None:
    ...
    logger.add(
        ...
        format="<green>{time:DD-MM-YYYY HH:mm:ss}</green> | <level>{level: <8}</level> | <cyan>{name}</cyan>:<cyan>{function}</cyan>:<cyan>{line}</cyan> | {message}",
        colorize=True,
    )
    ...
    logger.add(
        ...
        retention="30 days",
        level="DEBUG",
        format="{time:DD-MM-YYYY HH:mm:ss.SSS} | {level: <8} | {name}:{function}:{line} | {message} | {extra}",
        backtrace=True,
        diagnose=True,
    )
